...
UPDATED: 23-0905 - by: Joe Negron <joe.negron.nyc@gmail.com> - add windows ports & flagged last-hop for removal
VERSION: v0.8 - add ports for RPC, WinRM, legacy NetBIOS over TCPIP; Dark-Deprecation Last-Hop - too slow & not much value    
...
def infer_os(ports):
    ...
    if 3389 in ports and 22 not in ports:
        return f"{FG_BLU}Windows{RESET}"
    elif 22 in ports and 3389 not in ports:
        return f"{FG_GRN}Linux  {RESET}"
    elif 22 in ports and 3389 in ports:
        return f"{FG_YEL}Hybrid {RESET}" # Hybrid= both Windows & Linux
    else:
        return f"{FG_GRY}Unknown{RESET}"

# 24-0911JN-MOD: Dark-Deprecation Last-Hop - too slow & not much value
def get_last_hop(ip):
    """Performs a traceroute to the IP and finds the last hop before the destination."""
    try:
        result = subprocess.check_output(['traceroute', '-n', ip], stderr=subprocess.STDOUT, timeout=12).decode()
        hops = re.findall(r'(\d+\.\d+\.\d+\.\d+)', result)
        if len(hops) > 1:
            return hops[-2]  # Last hop before the target IP should be the gateway
        return "UNKNOWN"
...
def print_results(active_ports):
    ...
    for ip, ports in active_ports.items():
        if ports:
            ...
            # Format and print the line with aligned columns
            COLOR = FG_BLU if "IN-ADDR.ARPA" not in hostname else FG_ORN
            ...
            print(f"{CLOBBER}IP: {FG_GRN}{ip:<{padding1}}{RESET} \tDNS: {COLOR}{hostname:<{padding2}}{RESET} \tOS: {os_type} \tLISTENERS: {FG_RED}{', '.join(map(str, ports))}{RESET}")
...
if __name__ == "__main__":
    ...
    # Initiate the IP scan and display progress
    active_ports = scan_ips(ip_range, threads)

    # Reporting the results for all IPs
    print_results(active_ports)
